...
import numpy as np
import find_lambda_lasso as fl
...
if __name__ == '__main__':
    ...
    ds.div_and_scale()
    X_train, y_train = pd.prepare_data("../resources/data/training.data", function_type, 1)
    X_test, y_test = pd.prepare_data("../resources/data/testing.data", function_type, 1)

    costs = []
    sizes = [0.01, 0.02, 0.03, 0.075, 0.125, 0.25, 0.5, 0.625, 0.75, 0.875, 1]
    lambda_ = fl.find_lambda(function_type, 10)
    print(lambda_)
    with open("../resources/lasso/lasso_costs/lasso_cost_gaussian.txt", "a") as result_file:
        ...
        for size in sizes:
            ...
            theta_min = cd.coord_descent(X_present, y_present, lambda_, 1000)
